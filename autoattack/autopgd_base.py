    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if self.norm == 'L1':
            ...
            if x_init is None:
                ...
                topk = L0_norm(x_adv - x).type(torch.float32) / n_fts / 1.5
            ...
            with torch.no_grad():
              ...
              if counter3 == k:
                  if self.norm in ['Linf', 'L2']:
                      ...
                      fl_redtopk = (sp_curr.type(torch.float32) / sp_old.type(torch.float32)) < .95
                      topk = sp_curr.type(torch.float32) / n_fts / 1.5
